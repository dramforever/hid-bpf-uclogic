...
pub(crate) fn find_sysfs() -> Result<PathBuf> {
    if fstype(c"/sys")? != libc::SYSFS_MAGIC {
        bail!("/sys is not sysfs, please mount it with: mount -t sysfs sysfs /sys");
    }
    Ok("/sys".into())
}

pub(crate) fn find_bpffs() -> Result<PathBuf> {
...
pub(crate) fn find_subsystem(sysfs: &Path, subsystem: &str) -> Result<Option<PathBuf>> {
...
pub(crate) fn subsystem_devices(subsystem: &Path) -> Result<Vec<PathBuf>> {
...
pub(crate) fn property(device: &Path, name: &str) -> Result<Option<String>> {
    Ok(map_not_found(fs::read_to_string(device.join(name)))?)
}

pub(crate) fn property_trim(device: &Path, name: &str) -> Result<Option<String>> {
    property(device, name).map(|m| m.map(|s| s.trim().to_owned()))
}

pub(crate) fn get_subsystem(device: &Path) -> Result<Option<String>> {
...
pub(crate) fn find_map_parent<T>(
    device: &Path,
    mut f: impl FnMut(&Path) -> Result<Option<T>>,
) -> Result<Option<T>> {
...
pub(crate) fn hid_device_id(device: &Path) -> Result<i32> {
